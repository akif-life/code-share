...
app.get("/", async (req, res) => {
  ...
  res.render("index", {
    ...
    reqAdmin: req.user ? client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id).roles.cache.get(conf.adminRole) : null,
    Owenrrr: req.user ? client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id).roles.cache.get(conf.ownerRole) : null
  });
});
...
app.get("/profile/:userID", async (req, res) => {
  ...
  const codeData = require("./schemas/code");
...
app.get("/abonekodpaylas", async (req, res) => {
...
app.get("/oyunpaylas", async (req, res) => {
  if (!req.user || !client.guilds.cache.get(conf.guildID).members.cache.has(req.user.id)) return error(res, 138, "Kod paylaşabilmek için Discord sunucumuza katılmanız ve siteye giriş yapmanız gerekmektedir.");
  res.render("gameshare", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    isStaff: client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id).roles.cache.has(conf.ownerRole),
    reqMember: req.user ? client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id) : null,
...
app.get("/sablonpaylas", async (req, res) => {
  if (!req.user || !client.guilds.cache.get(conf.guildID).members.cache.has(req.user.id)) return error(res, 138, "Kod paylaşabilmek için Discord sunucumuza katılmanız ve siteye giriş yapmanız gerekmektedir.");
  res.render("tampleteshare", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    isStaff: client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id).roles.cache.has(conf.ownerRole),
    reqMember: req.user ? client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id) : null,
...
app.post("/abonesharing", async (req, res) => {
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  ...
  let newCodeData = new codeData({
    ...
    glitch: code.glitch.trim(),
    download: code.download.trim(),
    rank: code.rank,
    date: code.date
  }).save();
  ...
  let color;
  if (code.rank === "abone") color = "#f80000";
...
app.post("/gamesharing", async (req, res) => {
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  if (!req.user || !member) return error(res, 138, "Kod paylaşabilmek için Discord sunucumuza katılmanız ve siteye giriş yapmanız gerekmektedir.");
  const codeData = require("./schemas/code");
  ...
  code.sharers = code.sharers.trim().split(" ").filter(x => guild.members.cache.get(x));
  if (code.sharers && !code.sharers.includes(req.user.id)) code.sharers.unshift(req.user.id);
  if (!code.game || code.game && (code.game.trim().toLowerCase() === "yok" || code.game.trim() === "-")) code.game = "";
  cooldown.get(req.user.id) ? cooldown.set(req.user.id, { count: cooldown.get(req.user.id).count += 1 }) : cooldown.set(req.user.id, { count: 1 });
  if (await cooldown.get(req.user.id).count === 1) setTimeout(() => cooldown.delete(req.user.id), 1000*60*10);
...
app.post("/templatesharing", async (req, res) => {
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  if (!req.user || !member) return error(res, 138, "Kod paylaşabilmek için Discord sunucumuza katılmanız ve siteye giriş yapmanız gerekmektedir.");
  const codeData = require("./schemas/code");
  ...
  const channel = guild.channels.cache.get(conf.codeLog);
  let color;
  if (code.rank === "şablon") color = "#f80000";
  else if (code.rank === "fromyou") color = ""
  const embed = new MessageEmbed()
  .setAuthor(req.user.username, member.user.avatarURL({ dynamic: true }))
  .setThumbnail(guild.iconURL({ dynamic: true }))
  .setTitle(`${code.rank} kategorisinde bir kod paylaşıldı!`)
  .setDescription(`
  • Şablon adı: [${code.name}](https://${conf.domain}/${code.rank}/${id})
  • Şablon Açıklaması: ${code.desc}
  • Şablonu paylaşan: ${member.toString()}
  `)
  .setColor(color)
  channel.send(embed);
  res.redirect(`/${code.rank}/${id}`);
...
app.get("/template", async (req, res) => {
  const codeData = require("./schemas/code");
  const data = await codeData.find({ rank: "template" }).sort({ date: -1 });
...
app.get("/game", async (req, res) => {
  const codeData = require("./schemas/code");
  const data = await codeData.find({ rank: "game" }).sort({ date: -1 });
  res.render("oyunlar", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    data,
...
app.get("/abone", async (req, res) => {
  ...
  res.render("abone", {
    ...
    guild: client.guilds.cache.get(conf.guildID),
    reqMember: req.user ? client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id) : null,
    reqRole: req.user ? client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id).roles.cache.get(conf.abone) : null,
    reqAdmin: req.user ? client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id).roles.cache.get(conf.adminRole) : null,
    Owenrrr: req.user ? client.guilds.cache.get(conf.guildID).members.cache.get(req.user.id).roles.cache.get(conf.ownerRole) : null
  });
});
...
app.get("/template/:codeID", async (req, res) => {
  if (!req.user || !client.guilds.cache.get(conf.guildID).members.cache.has(req.user.id)) return error(res, 138, "Kodları görebilmek için Discord sunucumuza katılmanız ve siteye giriş yapmanız gerekmektedir.");
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  if (member && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu kodu görebilmek için gerekli rolleriniz bulunmamaktadır! Lütfen bilgilendirme sayfasını okuyunuz!");
  ...
  if (!codeID) return res.redirect("/");
  const codeData = require("./schemas/code");
  const code = await codeData.findOne({ rank: "template", id: codeID });
  if (!code) return error(res, 404, codeID+" ID'li bir kod bulunmuyor!");
  let axios = require('axios')
  let { tdata } = await axios.get('https://discord.com/api/v7/guilds/templates/kASEzRw5uSaR');
  res.render("code", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
    data: code,
...
app.get("/game/:codeID", async (req, res) => {
  if (!req.user || !client.guilds.cache.get(conf.guildID).members.cache.has(req.user.id)) return error(res, 138, "Kodları görebilmek için Discord sunucumuza katılmanız ve siteye giriş yapmanız gerekmektedir.");
  const guild = client.guilds.cache.get(conf.guildID);
  const member = req.user ? guild.members.cache.get(req.user.id) : null;
  const codeID = req.params.codeID;
  ...
  if (member && !member.roles.cache.has(conf.abone) && !member.roles.cache.has(conf.booster) && !member.roles.cache.has(conf.ownerRole) && member.roles.cache.has(conf.adminRole)) return error(res, 501, "Bu kodu görebilmek için gerekli rolleriniz bulunmamaktadır! Lütfen bilgilendirme sayfasını okuyunuz!");
  const codeData = require("./schemas/code");
  const code = await codeData.findOne({ rank: "game", id: codeID });
  if (!code) return error(res, 404, codeID+" ID'li bir kod bulunmuyor!");
  res.render("code", {
    user: req.user,
    icon: client.guilds.cache.get(conf.guildID).iconURL({ dynamic: true }),
...
client.on("ready", () => {
  ...
  console.log('\x1b[36m%s\x1b[0m', 'Developed by Gweep Creative');
});
